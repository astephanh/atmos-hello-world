locals {
  ...
  ds_configurations = {
    ...
    s3 = local.ds_backend != "s3" ? null : {
      ...
      # Use assume_role to access the remote state if the component is not privileged and `assume_role` is specified
      assume_role = !coalesce(try(local.backend.privileged, null), var.privileged) && contains(keys(local.backend), "assume_role") ? local.backend.assume_role : null

      # Deprecated: Use the role to access the remote state if the component is not privileged and `role_arn` is specified
